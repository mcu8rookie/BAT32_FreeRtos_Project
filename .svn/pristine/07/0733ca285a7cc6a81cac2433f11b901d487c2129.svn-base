...
//-----------------------------------------------------------------------
// CMP setting
// No.60 pin (P02/VCIN10) used as +side of CMP1
// BGR Voltage(1.45V)     used as -side of CMP1
//-----------------------------------------------------------------------
	CMP_Init(CMP_CHANNEL_1, CMP_PSIDE_VCIN10, CMP_NSIDE_BGR);
	CMP_Filter_Setting(CMP_CHANNEL_1, CMP_FILTER_FCLK_32, CMP_EDGE_RISING);
	PORT->PMC0 &= ~(1 << 2);  /* P02 digital function */
	PORT->PM0  &= ~(1 << 2);  /* P02 output mode */
	PORT->P0   &= ~(1 << 2);  /* P02 output low level */
	CMP_Start(CMP_CHANNEL_1);
	
//-----------------------------------------------------------------------
// TMC start
//-----------------------------------------------------------------------
	TMC_Init(TMC_CLOCK_FCLK);
...
//-----------------------------------------------------------------------
// TMC Capture by CMP1 interrupt
//-----------------------------------------------------------------------
	PORT->P0   |= (1 << 2);  /* P02 output high level */
	delayMS(1);
	width = TMC_Get_BufferValue();
	printf("TMC Capture Value = %d\n", width);
}
/***********************************************************************************************************************
