...
{   
    ...
    if((g_iica0_rx_end == 1)&&(g_iica0_slave_status_flag == 1))
    {
        if(I2CA_Cmd_Code == 0xD0)
        {
            Init_printf("\nI2CA RX_Len, %d, CmdCode, 0x%02X, ",g_iica0_rx_cnt,I2CA_Cmd_Code);
    ...
    if((g_iica0_tx_end == 1)&&(g_iica0_slave_status_flag == 1))
    {
        if(g_iica0_rx_end == 1)
        {
