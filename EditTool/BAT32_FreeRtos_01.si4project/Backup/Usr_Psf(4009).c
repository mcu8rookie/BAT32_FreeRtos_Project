...
int16_t ASC_Humi_RateMax;
int16_t ASC_Humi_Thre;


//#define DEF_ASC_ADJUST_VALUE_MAX        3
uint8_t ASC_Adjust_Flag;
uint8_t ASC_Adjust_Index;
...
{   
    ...
    {   // if self-monitoring function is enable; 
        ...
        if(ASC_Tmpr_Index>=DEF_TMPR_BUFFLEN)
        {
            ...
            if(ASC_Tmpr_Rate > ASC_Tmpr_RateMax)
            {
                ASC_Tmpr_RateMax = ASC_Tmpr_Rate;
            }
        ...
        if(ASC_Humi_Index>=DEF_HUMI_BUFFLEN)
        {
            ...
            if(ASC_Humi_Rate > ASC_Humi_RateMax)
            {
                ASC_Humi_RateMax = ASC_Humi_Rate;
            }
        ...
        {   // if add data keep 1 hour timeout;
            ...
            if((SelfMoni2_Leakage_Flag == 0)\
                &&((ASC_Tmpr_Min30M>=150)&&(ASC_Tmpr_Min30M<=35)&&(ASC_Tmpr_RateMax<10))\
                &&((ASC_Humi_Min30M>=300)&&(ASC_Humi_Min30M<=700)&&(ASC_Humi_RateMax<30)))
            {   
