...
{   
    ...
    {   // if self-monitoring function is enable; 
        ...
        //if(ASC_TimeCnt>=600)    // 5minute;
        //if(ASC_TimeCnt>=180)    // 3minute;
        {   // if add data keep 1 hour timeout;
            
            //if(SelfMoni2_Leakage_Flag == 0)
            if((SelfMoni2_Leakage_Flag == 0)\
                &&((ASC_Tmpr_Min30M>=150)&&(ASC_Tmpr_Max30M<=350)&&(ASC_Tmpr_RateMax30M<ASC_Tmpr_RateMax))\
                &&((ASC_Humi_Min30M>=300)&&(ASC_Humi_Max30M<=700)&&(ASC_Humi_RateMax30M<ASC_Humi_RateMax)))
            {   
                ...
                {   
                    ...
                    if(loc_cnt==ASC_ARRAYLEN)
                    {
                        ...
                        {   // Here some variables update to do;
