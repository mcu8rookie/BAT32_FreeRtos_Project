...
void Usr_DFData_To_Variable(void)
{
    ...
    #if(defined(DEF_ASC_EN)&&(DEF_ASC_EN==1))
    {
        ...
        ASC_Tmpr_RateMax = DF_Data[DEF_ASC_TMPR_THRE_INDEX+1];
        ASC_Tmpr_RateMax <<= 8;
        ASC_Tmpr_RateMax += DF_Data[DEF_ASC_TMPR_THRE_INDEX];
        
        ASC_Humi_RateMax = DF_Data[DEF_ASC_HUMI_THRE_INDEX+1];
        ASC_Humi_RateMax <<= 8;
        ASC_Humi_RateMax += DF_Data[DEF_ASC_HUMI_THRE_INDEX];
        ...
        if(ASC_Func_En==1)
        {
            ...
            if(((ASC_Tmpr_RateMax==0)||((uint16_t)ASC_Tmpr_RateMax==0xFFFF))\
            ||((ASC_Humi_RateMax==0)||((uint16_t)ASC_Humi_RateMax==0xFFFF)))
            {
                ASC_Func_En = 0;
            }
        }
