...
{   
    ...
    {   // if self-monitoring function is enable; 
        ...
        if((ASC_Stage==0)&&(ASC_Fast_DataCnt==8))
        {
            ASC_Init_Value = ASC_PPM_Total/ASC_PPM_Cnt;
            
            if((ASC_Tmpr_RateMax30M<ASC_Tmpr_RateTh)&&(ASC_Humi_RateMax30M<ASC_Humi_RateTh))
            {
                ASC_Init_Error = 1;
            }
            else
        ...
        if((ASC_Init_Error == 1)||(ASC_Init_Error==2))
        {
            ASC_TimeCnt_Th = 30;   // Data Cycle, 30 sec;
        }
        else
        {
