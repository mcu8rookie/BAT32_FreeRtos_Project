...
void Usr_DFData_To_Variable(void)
{
    ...
    #if 0 //(defined(DEF_HUMCOMP_EN)&&(DEF_HUMCOMP_EN == 1))
    {   
        unsigned char i,j;
        unsigned char *pbyte;
        ...
        {   
            ...
            for(j=0;j<4;j++)
            {
                *(pbyte+j) = Eeprom_ReadByte(EEPROM_PARA_SX_ADDRSTART+i*4+j);
            }
        }
        
        HumComp_Flag = Eeprom_ReadByte(EEPROM_PARA_SF_ADDRSTART+1);
        HumComp_Flag <<= 8;
        HumComp_Flag += Eeprom_ReadByte(EEPROM_PARA_SF_ADDRSTART);
