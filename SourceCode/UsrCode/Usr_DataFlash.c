...
void Usr_DFData_To_Variable(void)
{
	...
	if(FP32_IsNumerical((uint8_t *)(&g_TCompCoeff.P0))
		...
		&& FP32_IsNumerical((uint8_t *)(&g_TCompCoeff.P3)))
	{
		g_TCompCoeff.flag = 1;
	}
	else
    ...
    HeatBoard_PreTime = DF_Data[DEF_HEAT_BOARD_PREHEAT_INDEX+1];
    HeatBoard_PreTime <<= 8;
    HeatBoard_PreTime += DF_Data[DEF_HEAT_BOARD_PREHEAT_INDEX];
